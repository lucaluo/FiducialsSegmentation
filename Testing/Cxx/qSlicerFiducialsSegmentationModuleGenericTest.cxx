...
//-----------------------------------------------------------------------------
int qSlicerFiducialsSegmentationModuleGenericTest( int argc, char * argv[] )
{
  qSlicerApplication app(argc, argv);
  app.errorLogModel()->setTerminalOutputs(ctkErrorLogModel::All);

  qSlicerFiducialsSegmentationModule * module = new qSlicerFiducialsSegmentationModule;
  ...
  if (module->title().isEmpty())
    ...
    std::cerr << "Line " << __LINE__
              << " - qSlicerFiducialsSegmentationModule::title() is empty." << std::endl;
    return EXIT_FAILURE;
    }
  ...
  if (module->categories().isEmpty())
    {
    std::cerr << "Line " << __LINE__
              << " - qSlicerFiducialsSegmentationModule::categories() is empty." << std::endl;
    return EXIT_FAILURE;
    }
  ...
  if (module->index() < -1 || module->index() > 1000)
    {
    std::cerr << "Line " << __LINE__
              << " - qSlicerFiducialsSegmentationModule::index() seems invalid." << std::endl;
    return EXIT_FAILURE;
    }
  ...
  //  {
  //  std::cerr << "Line " << __LINE__
  //            << " - qSlicerFiducialsSegmentationModule::helpText() is empty." << std::endl;
  //  return EXIT_FAILURE;
  //  }
  ...
  if (module->acknowledgementText().isEmpty())
    {
    std::cerr << "Line " << __LINE__
              << " - qSlicerFiducialsSegmentationModule::acknowledgementText() is empty." << std::endl;
    return EXIT_FAILURE;
    }
  ...
  if (module->contributors().isEmpty())
    {
    std::cerr << "Line " << __LINE__
              << " - qSlicerFiducialsSegmentationModule::contributors() is empty." << std::endl;
    return EXIT_FAILURE;
    }
  ...
  if (module->appLogic() != 0)
    {
    std::cerr << "Line " << __LINE__
              << " - qSlicerFiducialsSegmentationModule::initialize() failed." << std::endl;
    return EXIT_FAILURE;
    }
  ...
  if (!moduleRepresentation && !module->isHidden())
    ...
    std::cerr << "Line " << __LINE__
              << " qSlicerFiducialsSegmentationModule has no widget representation."
  ...
  if (module->mrmlScene() != scene.GetPointer())
    {
    std::cerr << "Line " << __LINE__
              << " - qSlicerFiducialsSegmentationModule::setMRMLScene() failed." << std::endl;
    return EXIT_FAILURE;
    }
  ...
  if (logic && logic->GetMRMLScene() != scene.GetPointer())
    {
    std::cerr << "Line " << __LINE__
              << " - qSlicerFiducialsSegmentationModule::setMRMLScene() failed to pass"
